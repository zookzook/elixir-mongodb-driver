defmodule Mongo.SessionTest do
  ...
  @tag :mongo_4_2
  test "commit_transaction", top do
    ...
    %{"_id" => _id, "name" =>  "Tom"} = Mongo.find_one(top.pid, coll, %{name: "Tom"}, session: session)
  ...
  test "check invalid unordered bulk with transaction", top do
    ...
    {:error, [result|_xs]} = Session.with_transaction(top.pid, fn opts ->
  ...
  @tag :mongo_4_2
  test "commit empty transaction", top do
    assert {:ok, :ok} = Session.with_transaction(top.pid, fn _opts ->
     {:ok, :ok}
    end, w: 1)
  end

  @tag :mongo_4_2
  test "abort empty transaction", top do
    assert {:error, :ok} = Session.with_transaction(top.pid, fn _opts ->
      {:error, :ok}
    end, w: 1)
  end

  @tag :mongo_4_2
  test "check ordered bulk with transaction", top do
    coll = unique_name()
    Mongo.insert_one(top.pid, coll, %{name: "Wuff"})
    Mongo.delete_many(top.pid, coll, %{})
    ...
    {:ok, result} = Session.with_transaction(top.pid, fn opts ->
      result = BulkWrite.write(top.pid, bulk, opts)
      {:ok, result}
    end, w: 1)
